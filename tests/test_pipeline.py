...
# ==========================================================
# CONFIGURATION DES CHEMINS - ADAPTÉE À VOTRE STRUCTURE
# ==========================================================
...
# ==========================================================
# FONCTIONS POUR CHARGER VOS DONNÉES
# ==========================================================
...
def load_data_splits():
    ...
    for split_name in ['train', 'val', 'test']:
        try:
            ...
            X_df = pd.read_csv(CSV_PATHS[split_name])

            # Charger les labels
            y_df = pd.read_csv(LABEL_PATHS[split_name])

            # Fusionner les données
            if 'patient_id' in X_df.columns and 'image_id' in X_df.columns:
                # Si les DataFrames ont des colonnes séparées pour patient_id et image_id
                if 'patient_id' in y_df.columns and 'image_id' in y_df.columns:
                    merged_df = pd.merge(X_df, y_df, on=['patient_id', 'image_id'])
                else:
                    # Supposer que l'ordre est le même
                    merged_df = X_df.copy()
                    if len(y_df.columns) == 1:  # Un seul label
                        merged_df['cancer'] = y_df.iloc[:, 0]
            else:
                # Si les DataFrames ont un index commun
                merged_df = X_df.copy()
                merged_df['cancer'] = y_df.iloc[:, 0] if len(y_df.columns) == 1 else y_df['cancer']

            splits[split_name] = merged_df
            print(f"✅ Loaded {split_name}: {len(merged_df)} samples")
...
def get_test_cases_from_data(splits, max_cases=5):
    """Extrait des cas de test à partir des données chargées"""
    test_cases = []

    for split_name, df in splits.items():
        if not df.empty and len(test_cases) < max_cases:
            # Prendre les premiers échantillons de chaque split
            samples = df.head(max_cases - len(test_cases))

            for _, row in samples.iterrows():
                if 'patient_id' in row and 'image_id' in row:
                    test_cases.append((row['patient_id'], row['image_id']))
                else:
                    # Si pas de colonnes spécifiques, utiliser l'index
                    test_cases.append((f"{split_name}_{_}", f"image_{_}"))

    # Si pas assez de cas, en créer quelques-uns manuellement
    while len(test_cases) < max_cases:
        test_cases.append((f"sample_{len(test_cases)}", f"image_{len(test_cases)}"))
...
def setup_pipeline(tmp_path_factory):
    """Initialize pipeline components avec votre structure de données"""
    # Charger les données
    splits = load_data_splits()

    # Utiliser le split train pour la configuration principale
    if not splits['train'].empty:
        csv_path = CSV_PATHS['train']
    else:
        # Fallback vers un autre split
        csv_path = next((path for path in CSV_PATHS.values() if path.exists()), CSV_PATHS['train'])

    # Dossier d'images principal (train)
    images_dir = IMAGE_PATHS['train']
    ...
    return dataset_manager, cropping, windowing, resampler, splits
...
def find_dicom_file(patient_id, image_id):
    """Trouve le fichier DICOM correspondant dans les dossiers d'images"""
    # Essayer différents patterns de noms de fichiers
    possible_patterns = [
        f"{patient_id}_{image_id}.dcm",
        f"{image_id}.dcm",
        f"{patient_id}.dcm",
        f"{image_id}",
        f"{patient_id}"
    ...
    for split_name, image_dir in IMAGE_PATHS.items():
        if image_dir.exists():
            for pattern in possible_patterns:
                dicom_path = image_dir / pattern
                if dicom_path.exists():
                    return dicom_path, split_name

                # Chercher avec glob pour plus de flexibilité
                matching_files = list(image_dir.glob(f"*{image_id}*"))
                if matching_files:
                    return matching_files[0], split_name

                matching_files = list(image_dir.glob(f"*{patient_id}*"))
                if matching_files:
                    return matching_files[0], split_name
...
def process_single_image(
    ...
    try:
        # Trouver le fichier DICOM
        dicom_path, split_name = find_dicom_file(patient_id, image_id)
        if not dicom_path:
            raise FileNotFoundError(f"DICOM file not found for patient {patient_id}, image {image_id}")

        print(f"     📁 Found DICOM: {dicom_path} (split: {split_name})")

        # Get metadata from dataset manager
...
def test_pipeline_custom_patients(setup_pipeline):
    """
    Test the full pipeline on your custom data splits.
    """
    dataset_manager, cropping, windowing, resampler, splits = setup_pipeline

    print(f"\n{'=' * 70}")
    print(f"🚀 CUSTOM MULTI-PATIENT PIPELINE TEST - YOUR DATA STRUCTURE")
    print(f"{'=' * 70}")
    ...
    for split_name, df in splits.items():
        if not df.empty:
            ...
            if 'cancer' in df.columns:
                cancer_count = df['cancer'].sum() if df['cancer'].dtype in [int, bool] else 0
                print(f"   🎯 Cancer cases: {cancer_count}")

    # Générer les cas de test à partir des données
    TEST_CASES = get_test_cases_from_data(splits, max_cases=5)
    ...
    summary = {
        ...
        'data_splits_loaded': {k: len(v) for k, v in splits.items() if not v.empty}
    }
    ...
    for split_name, count in summary['data_splits_loaded'].items():
        print(f"📊 {split_name.upper()}: {count} samples loaded")
